...
export default function ManagePage(props) {
  ...
  async function getData() {
    const res = await axios.get(`${config.backend}/api/findbyrespo/${tempdata.value}`);
    res.status === 200 ? console.log("success") : console.log(`Error ${res.status}`);
    setData(res.data);
    setType(tempdata);
  }
  ...
  async function onNewPerson(person) {
    ...
    const res = await axios.put('http://localhost:3005/api/updaterespo',{personid: person.id, responsibility: type.value, hierarchy: person.manages})
    const res2 = await axios.put('http://localhost:3005/api/updateunit',{hierarchy: person.manages, akaUnit: person.currentUnit});
